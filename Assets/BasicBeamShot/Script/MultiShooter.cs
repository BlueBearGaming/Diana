...
public class MultiShooter : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Fire1"))
        {
            Bullet = Shot1;
            //Fire
            GameObject s1 = (GameObject)Instantiate(Bullet, this.transform.position, this.transform.rotation, transform);
            s1.GetComponent<BeamParam>().SetBeamParam(this.GetComponent<BeamParam>());

            GameObject wav = (GameObject)Instantiate(Wave, this.transform.position, this.transform.rotation, transform);
